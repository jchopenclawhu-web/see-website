...
def fetch_vocabulary():
    """Fetch vocabulary data from App_data tabs in all Google Sheets"""
    ...
    for level_name, sheet_id in LEVELS.items():
        try:
            # Read from App_data tab, columns A through G (7 columns)
            ...
            words = []
            for row in values[1:]:  # Skip header row (row 0 = Date, English_Word, ...)
...
if __name__ == '__main__':
    print("Fetching vocabulary data from Google Sheets App_data tabs...")
    print("=" * 50)
